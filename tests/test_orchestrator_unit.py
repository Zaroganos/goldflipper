...
def create_mock_config():
    ...
    return {
        "strategy_orchestration": {"enabled": True, "mode": "sequential", "max_parallel_workers": 3},
...
def create_sample_play():
    ...
    return {
        ...
        "entry_point": {"stock_price": 100.0, "entry_stock_price": 100.0, "entry_premium": 1.55, "order_type": "market"},
        "take_profit": {
            "TP_type": "single",
            "stock_price": 105.0,
            "stock_price_pct": 5.0,
...
class TestOrderAction:
    ...
    def test_is_opening_closing(self):
        ...
        assert OrderAction.BUY_TO_OPEN.is_opening()
        assert OrderAction.SELL_TO_OPEN.is_opening()
        assert OrderAction.SELL_TO_CLOSE.is_closing()
        assert OrderAction.BUY_TO_CLOSE.is_closing()

    def test_is_long_short(self):
        from goldflipper.strategy.base import OrderAction

        assert OrderAction.BUY_TO_OPEN.is_long()
        assert OrderAction.SELL_TO_CLOSE.is_long()
        assert OrderAction.SELL_TO_OPEN.is_short()
        assert OrderAction.BUY_TO_CLOSE.is_short()
...
class TestPositionSide:
    ...
    def test_from_order_action(self):
        from goldflipper.strategy.base import OrderAction, PositionSide
...
class TestOrchestratorInitialization:
    ...
    def test_init_default_state(self):
        from goldflipper.strategy.orchestrator import OrchestratorState, StrategyOrchestrator
    ...
    def test_init_with_resources(self):
        ...
        orchestrator = StrategyOrchestrator(config=config, market_data=market, brokerage_client=client)
...
class TestOptionSwingsStrategy:
    ...
    def test_strategy_name(self):
        ...
        config = create_mock_config()
        strategy = OptionSwingsStrategy(config=config, market_data=create_mock_market_data(), brokerage_client=create_mock_client())
    ...
    def test_is_long_strategy(self):
        ...
        assert strategy.is_long_strategy()
    ...
    def test_validate_play_valid(self):
        ...
        assert strategy.validate_play(play)
    ...
    def test_validate_play_missing_field(self):
        ...
        assert not strategy.validate_play(play)
...
def run_tests():
    ...
    test_classes = [TestOrderAction, TestPlayStatus, TestPositionSide, TestOrchestratorInitialization, TestOptionSwingsStrategy]
