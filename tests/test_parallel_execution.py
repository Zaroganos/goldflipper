...
def test_orchestrator_initialization():
    ...
    from goldflipper.strategy.orchestrator import StrategyOrchestrator, ExecutionMode
...
def test_parallel_execution_timing():
    ...
    class TimingWrapper:
        ...
        def __call__(self, strategy):
            ...
            with lock:
                execution_times[self.strategy_name] = {
                    'start': start,
                    'end': end,
                    'duration': end - start,
                    'thread': thread_id
    ...
    if len(execution_times) < 2:
        ...
        print(f"   This may be expected if no plays are in the new/ directory")
    
    # Check for thread diversity (parallel should use different threads)
    threads_used = set(t.get('thread') for t in execution_times.values() if t.get('thread'))
    
    print(f"\nThreads used: {len(threads_used)}")
    for name, times in execution_times.items():
        print(f"  {name}: thread {times.get('thread', 'N/A')}, "
              f"duration {times.get('duration', 0):.3f}s")
