...
from goldflipper.utils.exe_utils import get_play_subdir, get_plays_dir
...
class PlayFileFixer:
    ...
    def _get_play_files(self):
        ...
        for dir_path in self.play_dirs:
            ...
            if dir_path.exists():
                all_play_files.extend([f for f in dir_path.glob("*.json") if f.is_file()])
    ...
    def _extract_play_info(self, content):
        ...
        return {"symbol": symbol, "trade_type": trade_type, "play_class": play_class}
            ...
            if self._is_cut_off_at_entry_premium(content):
                ...
                try:
                    ...
                    if "status" not in partial_json:
                        partial_json["status"] = {"play_status": "NEW", "order_id": None, "order_status": None, "position_exists": False}
