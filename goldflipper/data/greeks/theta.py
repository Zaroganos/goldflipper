...
class ThetaCalculator(GreeksCalculator):
    ...
    def calculate_call_theta(self) -> float:
        ...
        term1 = -(self.data.underlying_price * self.data.volatility * np.exp(-self.data.dividend_yield * self.data.time_to_expiry) * norm.pdf(d1)) / (
            2 * np.sqrt(self.data.time_to_expiry)
        )

        term2 = -self.data.risk_free_rate * self.data.strike_price * np.exp(-self.data.risk_free_rate * self.data.time_to_expiry) * norm.cdf(d2)

        term3 = self.data.dividend_yield * self.data.underlying_price * np.exp(-self.data.dividend_yield * self.data.time_to_expiry) * norm.cdf(d1)
