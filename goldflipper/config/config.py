    ...
    if not os.path.exists(config_path):
        if auto_create:
            ...
            try:
                ...
                raise OSError(f"Error creating settings file from template: {e}") from e
        else:
...
def reload_config(auto_create: bool = False) -> "Config":
    """
    ...
    if auto_create and not is_config_loaded():
...
class Config:
    ...
    def get(self, *keys: Any, default: Any = None) -> Any:
        ...
        # Backward-compatible support for positional default:
        # config.get("section", "key", fallback)
        if resolved_keys and resolved_default is None and not isinstance(resolved_keys[-1], str):
            resolved_default = resolved_keys.pop()
        ...
        return resolved_default if value is None else value
...
def get_active_account_name() -> str:
    ...
    """
    account_name = config.get("alpaca", "active_account", default="paper_1")
    return str(account_name) if account_name is not None else "paper_1"
...
def get_enabled_accounts() -> list:
    ...
    """
    accounts_obj = config.get("alpaca", "accounts", default={})
...
def get_account_nickname(account_name: str) -> str:
    ...
    """
    nickname = config.get("alpaca", "accounts", account_name, "nickname", default=account_name)
    return str(nickname) if nickname is not None else account_name
