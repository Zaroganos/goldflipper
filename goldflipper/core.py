...
from alpaca.trading.requests import ClosePositionRequest, GetOptionContractsRequest, LimitOrderRequest, MarketOrderRequest
...
def get_stock_price(symbol: str) -> float | None:
    ...
    try:
        ...
        if price is not None:
            ...
            price_item = getattr(price, "item", None)
            if callable(price_item):
                raw_price = price_item()
...
def get_option_data(option_contract_symbol: str) -> dict[str, float] | None:
...
def evaluate_closing_strategy(symbol, play, play_file=None):
    ...
    return _shared_eval_closing(
        symbol, play, play_file, get_stock_price_fn=get_stock_price, get_option_data_fn=get_option_data, save_play_fn=save_play_improved
    )
...
def close_position(play, close_conditions, play_file):
    ...
    try:
        ...
        close_type = "TP" if close_conditions["is_profit"] else "SL(C)" if close_conditions["is_contingency_loss"] else "SL"
        ...
        if close_conditions["is_profit"]:
            ...
            if play["take_profit"].get("order_type", "").lower().startswith("limit"):
                if limit_price is None:
                    logging.error("Take profit limit order requested but no limit price is available.")
                    return False
                limit_price = round(limit_price, 2)
                order_req = LimitOrderRequest(
                ...
                play["status"].update(
                    {"closing_order_id": str(response.id), "closing_order_status": response.status, "play_status": "PENDING-CLOSING"}
                )
                save_play(play, play_file)
                ...
                response = client.close_position(symbol_or_asset_id=contract_symbol, close_options=ClosePositionRequest(qty=str(qty)))
        ...
        # Stop loss handling
            ...
            if play["stop_loss"].get("SL_type", "").upper() == "CONTINGENCY":
                ...
                # If only primary condition is met, use limit order
                    ...
                    if limit_price is None:
                        logging.error("Contingency stop loss limit order requested but no limit price is available.")
                        return False
                    limit_price = round(limit_price, 2)
                    order_req = LimitOrderRequest(
                        symbol=contract_symbol,
            ...
            # Now handle regular (non-contingency) limit stop loss
                ...
                # Round limit price to 2 decimal places
                if limit_price is None:
                    logging.error("Stop loss limit order requested but no limit price is available.")
                    return False
                limit_price = round(limit_price, 2)
                order_req = LimitOrderRequest(
                    symbol=contract_symbol,
...
def monitor_and_manage_position(play, play_file):
    ...
    try:
        ...
        if (
            play["take_profit"].get("stock_price") is not None
            or play["stop_loss"].get("stock_price") is not None
            or play["take_profit"].get("stock_price_pct") is not None
            or play["stop_loss"].get("stock_price_pct") is not None
        ):
            try:
...
def validate_market_hours():
    ...
    if is_market_holiday(current_time.date()):
        ...
        try:
            ...
            next_market_day = next_market_day.replace(hour=int(start_time.split(":")[0]), minute=int(start_time.split(":")[1]), second=0)
...
def handle_conditional_plays(play, play_file):
    ...
    display.status(f"| OCO | OSO |: evaluating trigger conditions for {play.get('symbol', '')} {play.get('option_contract_symbol', '')}")
    ...
    for oco_trigger in oco_triggers:
        ...
        if os.path.exists(pending_opening_path):
            try:
                ...
                with open(pending_opening_path) as f:
...
def handle_end_of_day_pending_plays():
    ...
    try:
        ...
        if os.path.exists(pending_opening_dir):
            play_files = [os.path.join(pending_opening_dir, f) for f in os.listdir(pending_opening_dir) if f.endswith(".json")]
            ...
            for play_file in play_files:
                try:
                    ...
                    play["status"].update({"order_id": None, "order_status": None, "play_status": "NEW"})
        ...
        if os.path.exists(pending_closing_dir):
            play_files = [os.path.join(pending_closing_dir, f) for f in os.listdir(pending_closing_dir) if f.endswith(".json")]
            ...
            for play_file in play_files:
                try:
                    ...
                    # Update play status before moving
                    play["status"].update({"closing_order_id": None, "closing_order_status": None, "play_status": "OPEN"})
...
def manage_pending_plays(plays_dir: str | None, single_play: tuple[dict[str, Any], str] | None = None) -> bool:
    """
    ...
    if single_play:
        ...
        if plays_dir is None:
            logging.error("manage_pending_plays requires plays_dir when single_play is not provided")
            return False
        display.header("Managing pending plays...")
        logging.info("Managing pending plays")

    base_plays_dir = plays_dir if plays_dir is not None else ""
    ...
    for pending_type in ["pending-opening", "pending-closing"]:
        ...
        if single_play:
            ...
            if play.get("status", {}).get("play_status").lower() != pending_type:
                continue
            plays_to_process: list[tuple[dict[str, Any], str]] = [(play, play_file)]
        else:
            pending_dir = os.path.join(base_plays_dir, pending_type)
            if not os.path.exists(pending_dir):
                continue
            play_files = [os.path.join(pending_dir, f) for f in os.listdir(pending_dir) if f.endswith(".json")]
            plays_to_process = []
            for pf in play_files:
                loaded_play = load_play(pf)
                if isinstance(loaded_play, dict):
                    plays_to_process.append((loaded_play, pf))
