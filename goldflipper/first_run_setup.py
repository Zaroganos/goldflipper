...
class FirstRunSetup:
    ...
    def _browse_data_directory(self):
        ...
        dir_path = filedialog.askdirectory(title="Select Data Directory", mustexist=False)
        if dir_path:
    ...
    def show_setup_ui(self):
        ...
        if self.dnd_available and self.DND_FILES:
            ...
            try:
                ...
                dnd_entry.dnd_bind("<<Drop>>", self.handle_drop)
            except Exception as e:
...
'''
            ...
            ps_command = (
                f'$WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut("{shortcut_path}"); '
                f'$Shortcut.TargetPath = "{target_path}"; '
                f'$Shortcut.WorkingDirectory = "{working_dir}"; '
                '$Shortcut.Description = "Launch Goldflipper"; '
                "$Shortcut.Save()"
            )
            ...
            if result.returncode != 0:
                error_msg = result.stderr.strip() if result.stderr else result.stdout.strip()
                raise Exception(f"Failed to create shortcut: {error_msg}") from import_error

        except Exception as e:
            raise Exception(f"Error creating shortcut: {str(e)}") from e
