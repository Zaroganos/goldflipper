...
class CandlestickChart(BaseChart):
    ...
    def create(self) -> None:
        ...
        try:
            ...
            wick_width = width * 0.2
            
            # Plot up candlesticks
            self.axes[0].bar(up.index, up.Close - up.Open, bottom=up.Open, 
                            width=candle_width, color='green', alpha=0.8,
                            zorder=2)  # Higher zorder to appear above grid
            self.axes[0].vlines(up.index, up.Low, up.High, 
                               color='green', linewidth=1, zorder=1)
            
            # Plot down candlesticks
            self.axes[0].bar(down.index, down.Close - down.Open, bottom=down.Open,
                            width=candle_width, color='red', alpha=0.8,
                            zorder=2)
            self.axes[0].vlines(down.index, down.Low, down.High,
                               color='red', linewidth=1, zorder=1)
            
            # Plot volume with colors matching price movement
            self.axes[1].bar(up.index, up.Volume, width=candle_width, 
                            color='green', alpha=0.5)
            self.axes[1].bar(down.index, down.Volume, width=candle_width,
                            color='red', alpha=0.5)
