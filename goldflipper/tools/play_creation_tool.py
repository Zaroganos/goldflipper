...
def validate_choice(choice: str, options: list[str]) -> bool:
    """
...
def create_option_contract_symbol(symbol: str, expiration_date: str, strike_price: str | float, trade_type: str) -> str:
    """
    ...
    try:
        ...
        raise ValueError("Invalid expiration date format. Please use MM/DD/YYYY.") from e
    ...
    try:
        ...
        raise ValueError("Invalid strike price. Please enter a numeric value.") from e
...
def get_price_condition_type() -> list[int]:
    ...
    while True:
        ...
        try:
            ...
            return sorted(choices)  # Return sorted list of unique choices
...
def get_premium_percentage() -> float:
...
def get_order_type_choice(is_stop_loss: bool = False, transaction_type: str = "") -> str:
    ...
    if choice is None:
        return "limit at bid"
    return {1: "market", 2: "limit at bid", 3: "limit at last", 4: "limit at ask", 5: "limit at mid"}[choice]


def get_sl_type_choice() -> int:
    ...
    return get_input(
        ...
        error_message=(
            "Please enter 1 for 'Stop' (Market Order), 2 for 'Limit' (Limit Order), "
            "or 3 for 'Contingency' (Primary Limit Order with backup Market Order)."
        ),
    )


def validate_contingency_prices(main_price: float, backup_price: float, is_stock_price: bool, trade_type: str) -> bool:
    """
...
def get_multiple_tp_choice() -> bool:
...
class PlayBuilder:
    ...
    def build_base_play(self):
        ...
        raw_symbol = get_input(
            "Enter the ticker symbol (e.g., SPY or $SPY): ", str, validation=lambda x: len(x) > 0, error_message="Ticker symbol cannot be empty."
        )
        ...
        self.play["entry_point"]["stock_price"] = get_input(
            "Enter the entry stock price: ", float, error_message="Please enter a valid number for the entry price."
        )
    ...
    def _add_strike_price(self):
        ...
        self.play["strike_price"] = str(
            get_input("Enter the strike price: ", float, error_message="Please enter a valid number for the strike price.")
        )
    ...
    def _generate_contract_symbol(self):
        ...
        try:
            self.play["option_contract_symbol"] = create_option_contract_symbol(
                self.play["symbol"], self.play["expiration_date"], self.play["strike_price"], self.play["trade_type"]
            )
    ...
    def get_tp_parameters(self, tp_number=None, total_tps=None, total_contracts=None):
        ...
        tp_data["order_type"] = get_order_type_choice(transaction_type=f"Take Profit{f' #{tp_number}' if tp_number else ''}")
    ...
    def add_stop_loss(self):
        ...
        sl_data: dict[str, Any] = {"SL_type": "STOP" if sl_type == 1 else "LIMIT" if sl_type == 2 else "CONTINGENCY"}
        ...
        if sl_data["SL_type"] in ["STOP", "LIMIT"]:
            ...
            if 1 in sl_price_types:  # Absolute stock price
                ...
                while True:
                    ...
                    price_direction = "lower" if self.play["trade_type"] == "CALL" else "higher"
                    backup_stock_price = get_input(
                        f"Enter backup stop loss stock price ({price_direction} than {main_stock_price}): ",
                        float,
                        validation=lambda x: x > 0,
                    ...
                    if validate_contingency_prices(main_stock_price, backup_stock_price, True, self.play["trade_type"]):
                        break
                    TerminalDisplay.error(
                        f"Error: Backup price must be {price_direction} than the main price ({main_stock_price}) for a {self.play['trade_type']}",
            ...
            if 3 in sl_price_types:  # Stock price % movement
                ...
                while True:
                    ...
                    if backup_stock_price_pct > main_stock_price_pct:
                        break
                    TerminalDisplay.error(
                        f"Error: Backup percentage ({backup_stock_price_pct}%) must be higher than main percentage ({main_stock_price_pct}%)",
                        show_timestamp=False,
                    )
            ...
            if 2 in sl_price_types:  # Option premium %
                ...
                while True:
                    ...
                    if validate_contingency_prices(main_premium, backup_premium, False, self.play["trade_type"]):
                        break
                    TerminalDisplay.error(
                        f"Error: Backup loss percentage ({backup_premium}%) must be higher than "
                        f"main loss percentage ({main_premium}%) to represent a bigger loss",
                        show_timestamp=False,
                    )
    ...
    def _add_play_metadata(self):
        ...
        self.play["strategy"] = "Option Swings" if self.play["play_class"] == "SIMPLE" else "Branching Brackets Option Swings"
        ...
        self.play["play_expiration_date"] = get_input(
            ...
            str,
            validation=_is_valid_date,  # Validates date format
            error_message="Please enter a valid date in MM/DD/YYYY format.",
            optional=True,
        )
