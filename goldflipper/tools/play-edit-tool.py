...
def load_play(filepath: str) -> dict[str, Any] | None:
...
def save_play(filepath: str, play_data: dict[str, Any]) -> bool:
...
def get_all_plays() -> list[dict[str, str]]:
            ...
            if os.path.exists(folder_path):
                for file in os.listdir(folder_path):
                    if file.endswith(".json"):
                        ...
                        plays.append({"filepath": filepath, "folder": folder, "filename": file})
    else:
...
def find_play_file(filename: str) -> dict[str, str] | None:
    ...
    if USE_ACCOUNT_PATHS:
        ...
        for folder in folders:
            ...
            if os.path.exists(filepath):
                return {"filepath": filepath, "folder": folder, "filename": filename}
    else:
...
def edit_play_field(play_data, field, filepath):
    ...
    if field == "expiration_date":
        ...
        if not os.path.normpath(filepath).replace(os.sep, "/").split("/plays/")[1].startswith("new/"):
            print(
                "\nERROR: You are attempting to modify the Option Contract Expiration Date. "
                "This field should only be modified for a NEW play, in case you wish to change the expiration. "
                "If you wish to duplicate this play with a new expiration date, there is an option for that."
            )
            return

        confirm = get_input(
            "\nWARNING: Changing expiration date will affect the option contract symbol. "
            "DO NOT DO THIS unless you are absolutely sure you want to change the option's expiration date. "
            "Continue? (y/N): ",
            str,
            validation=lambda x: x.lower() in ["y", "n", ""],
            error_message="Please enter 'y' or 'n'",
        ...
        new_value = get_input(
            "Enter new expiration date (MM/DD/YYYY): ", str, validation=_is_valid_date, error_message="Please enter a valid date in MM/DD/YYYY format"
        )
        ...
        if new_value:
            ...
            if "play_name" in play_data and old_contract_symbol in play_data["play_name"]:
                play_data["play_name"] = play_data["play_name"].replace(old_contract_symbol, new_contract_symbol)
    ...
    # Play Expiration Date
        ...
        play_data["take_profit"] = {"stock_price": None, "stock_price_pct": None, "premium_pct": None, "order_type": "market"}

        if tp_price_type in [1, 4]:  # Absolute stock price
            price = get_input(
                "Enter take profit stock price: ", float, validation=lambda x: x > 0, error_message="Please enter a valid positive number"
            )
        ...
        if tp_price_type in [2, 4, 5]:  # Any condition involving premium %
            premium = get_input(
                "Enter take profit premium percentage: ", float, validation=lambda x: x > 0, error_message="Please enter a valid positive percentage"
            )
        ...
        if sl_type in ["STOP", "LIMIT"]:
            if price_type in [1, 4]:  # Absolute stock price
                price = get_input(
                    "Enter stop loss stock price: ", float, validation=lambda x: x > 0, error_message="Please enter a valid positive number"
                )
            ...
            if price_type in [1, 4]:  # Absolute stock price
                main_price = get_input(
                    "Enter main stop loss stock price: ", float, validation=lambda x: x > 0, error_message="Please enter a valid positive number"
                )
                backup_price = get_input(
                    "Enter backup stop loss stock price: ", float, validation=lambda x: x > 0, error_message="Please enter a valid positive number"
                )
    ...
    if field == "symbol":
        ...
        new_value = get_input("Enter new symbol: ", str, validation=lambda x: len(x) > 0, error_message="Symbol cannot be empty").upper()
        ...
        if new_value:
            ...
            play_data["option_contract_symbol"] = play_data["option_contract_symbol"].replace(old_symbol, new_value)

            # Update play name if it contains the old contract symbol
            if "play_name" in play_data and old_contract_symbol in play_data["play_name"]:
                play_data["play_name"] = play_data["play_name"].replace(old_contract_symbol, play_data["option_contract_symbol"])
        ...
        new_value = get_input(
            "Enter trade type (CALL or PUT): ", str, validation=lambda x: x.upper() in ["CALL", "PUT"], error_message="Please enter CALL or PUT"
        ).upper()
        ...
        if new_value:
            ...
            play_data["option_contract_symbol"] = play_data["option_contract_symbol"].replace(old_type, new_type)
...
def select_plays_from_list(plays, prompt=None):
    ...
    while True:
        choice = get_input(input_prompt, type_cast=str, optional=True)
        if not choice:
            break
...
def edit_conditional_plays(play_data, filepath):
    ...
    while True:
        ...
        choice = get_input("\nSelect option (1-3): ", type_cast=int, validation=lambda x: x in [1, 2, 3], error_message="Please enter 1, 2 or 3")
...
def edit_oco_trigger(play_data, filepath):
    ...
    if choice == 1:
        ...
        if current_triggers:
            ...
            to_remove = select_plays_from_list(trigger_dicts, prompt="\nEnter play number to remove (or press Enter to finish): ")
            if to_remove:
                play_data["conditional_plays"]["OCO_triggers"] = [t for t in current_triggers if t not in to_remove]
...
def edit_oto_trigger(play_data, filepath):
    ...
    if choice == 1:
        ...
        if current_triggers:
            ...
            to_remove = select_plays_from_list(trigger_dicts, prompt="\nEnter play number to remove (or press Enter to finish): ")
            if to_remove:
                play_data["conditional_plays"]["OTO_triggers"] = [t for t in current_triggers if t not in to_remove]
                print("\nSelected triggers removed")
        else:
...
def update_play_class(play_data):
    ...
    has_children = play_data["conditional_plays"].get("OCO_trigger") or play_data["conditional_plays"].get("OTO_trigger")
...
def edit_specific_play(file_arg):
    ...
    if os.path.isfile(file_arg):
        ...
        play_info = {"filepath": filepath, "folder": folder, "filename": filename}
    else:
    ...
    while True:
        ...
        max_selection = len(fields) + 2

        # Get user selection
        field_selection = get_input(
            "\nEnter the number of the field to edit or option to perform (0 to save and exit): ",
            type_cast=int,
            validation=lambda x, max_selection=max_selection: 0 <= x <= max_selection,
            error_message=f"Please enter a number between 0 and {max_selection}",
        )
...
def duplicate_play(play_data, original_filepath):
    ...
    folder_choice = get_input("Enter choice (1-2): ", int, validation=lambda x: x in [1, 2], error_message="Please enter 1 or 2")
...
def play_name(play_data, play_info):
    ...
    new_name = get_input("Enter new play name: ", str, validation=lambda x: len(x.strip()) > 0, error_message="Play name cannot be empty")
...
def get_price_condition_type():
    ...
    return get_input("Choice (1-5): ", int, validation=lambda x: x in [1, 2, 3, 4, 5], error_message="Please enter a number between 1 and 5")
