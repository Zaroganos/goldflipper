...
class AutoPlayCreator:
    def __init__(self):
        ...
        self.current_playbook: MomentumPlaybook | None = None
    ...
    def calculate_gap_info(self, symbol: str) -> dict[str, Any] | None:
        """
    ...
    def calculate_goldflipper_gap_data(self, symbol: str, current_price: float, calls_df, puts_df) -> dict[str, Any]:
        """
    ...
    def generate_tp_sl_values(self, entry_price, trade_type="CALL"):
        ...
        for tp_type in selected_types:
            if tp_type == "STOCK_PRICE":
                ...
                tp_values["stock_price"] = (
                    round(entry_price * (1 + tp_pct_val / 100), 2) if trade_type == "CALL" else round(entry_price * (1 - tp_pct_val / 100), 2)
                )
                sl_values["stock_price"] = (
                    round(entry_price * (1 - sl_pct_val / 100), 2) if trade_type == "CALL" else round(entry_price * (1 + sl_pct_val / 100), 2)
                )
    ...
    def _create_option_swings_play(self, market_data, trade_type="CALL"):
        ...
        entry_point = {"stock_price": round(entry_price, 2), "order_type": selected_order_type, "entry_premium": 0.0}
        ...
        take_profit = {
            **tp_values,  # Unpack all TP values
            ...
            "stock_price": tp_values.get(
                "stock_price", round(entry_price * (1 + ((self.take_profit_pct_sim if sim_mode else self.take_profit_pct) / 100)), 2)
            ),
        }
        ...
        stop_loss: dict[str, Any] = {
            **sl_values,  # Unpack all SL values
            ...
            "stock_price": sl_values.get(
                "stock_price", round(entry_price * (1 - ((self.stop_loss_pct_sim if sim_mode else self.stop_loss_pct) / 100)), 2)
            ),
        }
    ...
    def _create_momentum_play(self, market_data, trade_type="CALL"):
        ...
        if self.current_playbook == MomentumPlaybook.GOLDFLIPPER_GAP_MOVE:
            momentum_config["straddle_config"] = {"enabled": True, "min_gap_vs_straddle_ratio": 1.0}
            momentum_config["open_interest_config"] = {"enabled": True, "min_directional_oi": 500, "min_oi_ratio": 1.0}
        ...
        if self.current_playbook == MomentumPlaybook.GOLDFLIPPER_GAP_MOVE:
            ...
            goldflipper_gap_data = self.calculate_goldflipper_gap_data(
                symbol=symbol, current_price=current_price, calls_df=market_data.get("calls"), puts_df=market_data.get("puts")
            )
    ...
    def _create_sell_puts_play(self, market_data, trade_type="PUT"):
        ...
        # Collateral info
        collateral_info = {"required": True, "type": "cash", "amount": collateral, "calculated": True}
