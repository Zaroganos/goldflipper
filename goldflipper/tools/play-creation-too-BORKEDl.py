...
def create_play():
    ...
    while True:
        ...
        if multiple_tps_enabled and get_multiple_tp_choice():
            ...
            # Initialize stop_loss dictionary
            stop_loss_data = {
                'SL_type': None,
                'stock_price': None,
                'stock_price_pct': None,
            ...
            if stop_loss_data['SL_type'] in ['STOP', 'LIMIT']:
                ...
                if 3 in sl_price_types:  # Stock price % movement
                    stop_loss_data['stock_price_pct'] = get_input(
                ...
                if 2 in sl_price_types:  # Option premium %
                    stop_loss_data['premium_pct'] = get_premium_percentage()
                ...
                if 1 in sl_price_types:  # Absolute stock price
                    ...
                    stop_loss_data['stock_price'] = main_stock_price
                    stop_loss_data['contingency_stock_price'] = backup_stock_price
                ...
                if 3 in sl_price_types:  # Stock price % movement
                    ...
                    stop_loss_data['stock_price_pct'] = main_stock_price_pct
                    stop_loss_data['contingency_stock_price_pct'] = backup_stock_price_pct
                ...
                if 2 in sl_price_types:  # Option premium %
                    ...
                    stop_loss_data['premium_pct'] = main_premium
                    stop_loss_data['contingency_premium_pct'] = backup_premium
        ...
        if play['stop_loss']['SL_type'] == 'STOP':
            ...
            # Set order type(s) based on SL_type
            if stop_loss_data['SL_type'] == 'STOP':
                stop_loss_data['order_type'] = 'market'
            elif stop_loss_data['SL_type'] == 'LIMIT':
                stop_loss_data['order_type'] = get_order_type_choice(
                    is_stop_loss=True, 
                    transaction_type="Stop Loss"
                )
            else:  # CONTINGENCY
                stop_loss_data['order_type'] = ['limit', 'market']
        ...
        if play['play_class'] == 'OTO':
            ...
            play_status = 'NEW'
            # ... Get all stop loss parameters using existing logic ...
            # (Copy the stop loss parameter collection logic here)
            
            # Create each play with its own TP
            base_filename = sanitize_filename(play['play_name'])
            
            for i in range(num_tps):
                current_play = play.copy()
                current_play['take_profit'] = get_tp_parameters(i + 1, num_tps)
                current_play['stop_loss'] = stop_loss_data.copy()  # Use the collected stop loss data
                current_play['play_name'] = f"{play['play_name']}_{i+1}"
                
                # Determine directory and filename
                if current_play['play_class'] == 'OTO':
                    plays_dir = os.path.join(os.path.dirname(__file__), '..', 'plays', 'temp')
                    play_status = 'TEMP'
                else:
                    plays_dir = os.path.join(os.path.dirname(__file__), '..', 'plays', 'new')
                ...
                print(f"Play {i+1} saved to {filepath}")
            ...
            # Skip the rest of the regular play creation since we've handled everything
        ...
        # ... continue with existing single TP logic ...
