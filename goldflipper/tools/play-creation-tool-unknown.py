...
def translate_price_conditions(choices):
    """
    Translates multiple condition choices into the single number format used by the existing code.
    
    Old format:
    1 = Stock price only
    2 = Premium % only
    3 = Stock price % only
    4 = Stock price + Premium %
    5 = Stock price % + Premium %
    6 = All three conditions (Stock price + Premium % + Stock price %)
    
    Args:
        choices (list): List of selected choices (1, 2, and/or 3)
    
    Returns:
        int: Single number representing the combination in the old format
    """
    choices = set(choices)  # Convert to set for easier comparison
    
    if choices == {1, 2, 3}:  # All three conditions
        return 6
    elif choices == {1, 2}:  # Stock price + Premium %
        return 4
    elif choices == {2, 3}:  # Stock price % + Premium %
        return 5
    elif 1 in choices:  # Stock price only
        return 1
    elif 2 in choices:  # Premium % only
        return 2
    elif 3 in choices:  # Stock price % only
        return 3
    else:
        raise ValueError("Invalid combination of choices")
...
def get_price_condition_type():
    ...
    while True:
        ...
        try:
            return translate_price_conditions(choices)
...
def get_sl_type_choice():
    ...
    print("1. Stop Market (executes market order when price is hit)")
    print("2. Stop Limit (executes limit order when price is hit)")
    print("3. Contingency (primary limit order with backup market order)")
    
    return get_input(
        "Choice (1/2/3): ",
        int,
        validation=lambda x: x in [1, 2, 3],
        error_message="Please enter 1 for Stop Market, 2 for Stop Limit, or 3 for Contingency."
    )
...
def create_play():
    ...
    while True:
        ...
        play['take_profit'] = {}  # Initialize empty dict instead of one with null values
        ...
        if play['stop_loss']['SL_type'] == 'STOP':
            ...
            play['stop_loss']['order_type'] = ['limit', 'market']
