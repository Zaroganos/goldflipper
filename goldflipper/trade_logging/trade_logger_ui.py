...
class TradeLoggerUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(button_frame, text="Export", command=self.export_data).grid(row=0, column=0, pady=5, padx=2)
        ttk.Button(button_frame, text="Open Export Folder", command=self.open_export_folder).grid(row=0, column=1, pady=5, padx=2, sticky=tk.W)

        # Export options frame
        options_frame = ttk.LabelFrame(main_frame, text="Export Options", padding="5")
        options_frame.grid(row=1, column=0, sticky="we", pady=5)
        ...
        ttk.Checkbutton(format_frame, text="CSV", variable=self.export_csv).pack(side=tk.LEFT, padx=10)
        ttk.Checkbutton(format_frame, text="Excel", variable=self.export_excel).pack(side=tk.LEFT, padx=10)

        # Desktop option
        desktop_frame = ttk.Frame(options_frame)
        desktop_frame.pack(fill=tk.X, pady=5)

        ttk.Checkbutton(desktop_frame, text="Save to Desktop", variable=self.save_to_desktop).pack(side=tk.LEFT, padx=(0, 10))
        ...
        ttk.Checkbutton(
            backfill_frame,
            text="Enable Data Backfill (fetch missing data e.g. Greeks via API)",
            variable=self.enable_backfill,
            command=self.on_backfill_changed,
        ...
        ttk.Checkbutton(scope_frame, text="Include ALL folders (except Old)", variable=self.include_all_folders).pack(side=tk.LEFT, padx=(0, 10))
        ...
        self.strategy_combo = ttk.Combobox(
            filter_frame, textvariable=self.strategy_filter, values=self.available_strategies, state="readonly", width=20
        )
        ...
        stats_frame.grid(row=2, column=0, sticky="we", pady=10)
        self.update_summary_stats(stats_frame)

        # Refresh button
        ttk.Button(main_frame, text="Refresh Data", command=lambda: self.refresh_all_data(stats_frame)).grid(row=3, column=0, pady=5)
    ...
    def on_backfill_changed(self):
        ...
        try:
            ...
            self.logger = PlayLogger(
                base_directory=self.logger.base_directory, save_to_desktop=self.logger.save_to_desktop, enable_backfill=self.enable_backfill.get()
            )

            if self.enable_backfill.get():
                messagebox.showinfo(
                    "Backfill Enabled", "Data backfill is now enabled. The next refresh will attempt to fetch missing Greeks data from the API."
                )
            else:
                messagebox.showinfo("Backfill Disabled", "Data backfill is now disabled. Future refreshes will not fetch missing data from the API.")
