...
class DataBackfillHelper:
    ...
    def check_missing_data(self, play_data: dict[str, Any]) -> dict[str, bool]:
        ...
        missing["delta_atOpen"] = logging_data.get("delta_atOpen") is None or logging_data.get("delta_atOpen") == 0.0

        missing["theta_atOpen"] = logging_data.get("theta_atOpen") is None or logging_data.get("theta_atOpen") == 0.0

        # Check for missing pricing data
        missing["premium_atOpen"] = logging_data.get("premium_atOpen") is None and entry_point.get("entry_premium") is None

        missing["price_atOpen"] = (
            logging_data.get("price_atOpen") is None and entry_point.get("entry_stock_price") is None and entry_point.get("stock_price") is None
        )
        ...
        missing["any_opening_data"] = missing["has_opening_date"] or not missing["premium_atOpen"] or not missing["price_atOpen"]

        return missing

    def construct_option_symbol(self, play_data: dict[str, Any]) -> str | None:
        """
    ...
    def get_historical_option_data(self, option_symbol: str, historical_date: date) -> dict[str, float] | None:
        """
...
if __name__ == "__main__":
    ...
    sample_play_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "plays", "closed", "SPOT-call-1-20250820-0802.json")

    if os.path.exists(sample_play_path):
        with open(sample_play_path) as f:
