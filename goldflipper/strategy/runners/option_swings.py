    ...
    def evaluate_new_plays(self, plays: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for play in plays:
            ...
            try:
                ...
                should_enter = evaluate_opening_strategy(symbol=symbol, play=play, get_stock_price_fn=self._get_stock_price)

                if should_enter:
                    self.log_trade_action(
                        "ENTRY_SIGNAL",
                        play,
                        {"entry_point": play.get("entry_point", {}).get("stock_price"), "order_type": play.get("entry_point", {}).get("order_type")},
                    )
        ...
        for play in plays:
            ...
            try:
                ...
                if gtd_result.get("should_close", False):
                    ...
                    self.log_trade_action("EXIT_SIGNAL_GTD", play, {"reason": gtd_result.get("close_reason", "")})
                ...
                if close_conditions.get("should_close", False):
                    ...
                    self.log_trade_action(
                        ...
                        play,
                        {"is_profit": close_conditions.get("is_profit"), "sl_type": close_conditions.get("sl_type")},
                    )
    ...
    def _evaluate_dynamic_gtd(self, play: dict[str, Any], play_file: str) -> dict[str, Any]:
        ...
        try:
            ...
            for mr in result.get("method_results", []):
                self.logger.debug(f"[GTD] {mr['method']}: {mr.get('action', 'N/A')} - {mr.get('reason', '')}")
            ...
            if result.get("effective_date_changed", False) and result.get("effective_date"):
                ...
                try:
                    ...
                    self.logger.info(f"[GTD] Updated effective date to {result['effective_date']} for {play.get('symbol', '')}")
                except Exception as e:
                    self.logger.warning(f"[GTD] Failed to persist effective date: {e}")

            if result.get("should_close", False):
                self.logger.info(f"[GTD] CLOSE_NOW triggered for {play.get('symbol', '')}: {result.get('close_reason', '')}")
