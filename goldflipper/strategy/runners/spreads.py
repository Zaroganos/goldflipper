...
class SpreadLeg:
    ...
    def __init__(self, symbol: str, action: OrderAction, contracts: int = 1, ratio: int = 1):
        self.symbol = symbol
        self.action = action
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {"symbol": self.symbol, "action": self.action.value, "contracts": self.contracts, "ratio": self.ratio}

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "SpreadLeg":
        """Create leg from dictionary."""
        return cls(
            symbol=data["symbol"], action=OrderAction.from_string(data["action"]), contracts=data.get("contracts", 1), ratio=data.get("ratio", 1)
        )
...
class SpreadsStrategy(BaseStrategy):
    ...
    def get_spread_type(self, play: dict[str, Any]) -> SpreadType | None:
        """
    ...
    def get_closing_legs(self, play: dict[str, Any]) -> list[SpreadLeg]:
        ...
        for leg in entry_legs:
            ...
            closing_legs.append(SpreadLeg(symbol=leg.symbol, action=closing_action, contracts=leg.contracts, ratio=leg.ratio))
    ...
    def _get_current_spread_value(self, play: dict[str, Any]) -> float | None:
        """
        ...
        for play in plays:
            ...
            try:
                ...
                if not (price_low <= stock_price <= price_high):
                    self.logger.debug(f"Stock price {stock_price:.2f} outside entry range [{price_low:.2f}, {price_high:.2f}]")
                    continue
                    ...
                    if limit_price:
                        ...
                        if is_credit:
                            ...
                            if current_per_share < limit_price:
                                self.logger.debug(f"Credit spread value ${current_per_share:.2f} below limit ${limit_price:.2f}")
                                continue
                        else:
                            # Debit spread: We want to pay at most limit_price
                            if current_per_share > limit_price:
                                self.logger.debug(f"Debit spread cost ${current_per_share:.2f} above limit ${limit_price:.2f}")
                                continue

                # All conditions met
                spread_type = play.get("spread_type", "unknown")
                self.log_trade_action(
                    "ENTRY_SIGNAL", play, {"spread_type": spread_type, "stock_price": stock_price, "legs": len(play.get("legs", []))}
                )
        ...
        for play in plays:
            ...
            try:
                ...
                if tp_hit or sl_hit:
                    ...
                    self.log_trade_action(f"EXIT_SIGNAL_{close_type}", play, {"reason": close_reason, "pnl": f"${pnl:.2f}"})
