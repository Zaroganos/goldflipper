...
class SellPutsStrategy(BaseStrategy):
    ...
    def _evaluate_entry_conditions(self, play: Dict[str, Any]) -> bool:
        ...
        if condition_met:
            self.logger.info(
                f"[SELL_PUTS] Entry condition met: {symbol} @ ${current_price:.2f} "
            ...
            if option_symbol:
                ...
                if option_data:
                    ...
                    min_premium = self.get_playbook_setting(
                        play, 'entry.min_premium', 
                        play.get('entry_point', {}).get('min_premium', 0.50)
                    )
                    current_premium = option_data.get('bid', 0)
                    
                    if current_premium < min_premium:
                        self.logger.info(
                            f"Premium ${current_premium:.2f} below minimum ${min_premium:.2f}"
                        )
                        return False
        else:
            self.logger.debug(
                f"[SELL_PUTS] Entry not met: {symbol} @ ${current_price:.2f} "
                f"(target: ${lower_bound:.2f}-${upper_bound:.2f})"
            )
            ...
            try:
                ...
                if close_conditions.get('should_close', False):
                    ...
                    self.log_trade_action(f'EXIT_SIGNAL_{close_type}', play, {
                        ...
                        'reason': close_conditions.get('exit_reason')
                    })
    ...
    def _evaluate_exit_conditions(
        ...
        if dte_exit:
            ...
            result['exit_reason'] = f'DTE management - closing before expiration'
            return result
        ...
        if tp_pct and entry_premium > 0:
            ...
            if current_premium <= target_premium:
                ...
                result['exit_reason'] = (
                    f'Take Profit: Premium dropped from ${entry_premium:.2f} to '
                    f'${current_premium:.2f} ({profit_pct:.1f}% profit)'
                )
        ...
        if entry_premium > 0:
            ...
            if sl_pct:
                ...
                if current_premium >= sl_target:
                    ...
                    result['is_primary_loss'] = True
                    result['exit_reason'] = (
                        f'Stop Loss: Premium rose from ${entry_premium:.2f} to '
                        f'${current_premium:.2f} ({loss_pct:.1f}% loss)'
                    )
            ...
            # Method 2: Max loss multiple (e.g., 2x credit received)
                ...
                if current_premium >= max_loss_premium:
                    ...
                    result['is_primary_loss'] = True
                    result['exit_reason'] = (
                        f'Max Loss Multiple: Premium at ${current_premium:.2f} '
                        f'exceeds {max_loss_multiple}x credit (${max_loss_premium:.2f})'
                    )
        ...
        self.logger.debug(
            f"[SELL_PUTS] Hold {symbol}: premium ${current_premium:.2f} "
    ...
    def _check_itm_status(self, play: Dict[str, Any]) -> bool:
        ...
        if roll_if_itm is None:
            roll_if_itm = self.get_playbook_setting(
                play, 'sell_puts_config.roll_if_itm', True
            )
        ...
        if current_price < strike_price:
            self.logger.warning(
                f"Put ITM: {symbol} @ ${current_price:.2f} < strike ${strike_price:.2f}"
            )
            return True
    ...
    def validate_play(self, play: Dict[str, Any]) -> bool:
        ...
        if play.get('trade_type', '').upper() != 'PUT':
            self.logger.warning(f"sell_puts strategy requires trade_type: PUT")
            return False
