...
def _update_high_water_marks(play: dict[str, Any], current_stock_price: float | None, current_premium: float | None) -> None:
...
def _activation_met(play: dict[str, Any], current_stock_price: float | None, current_premium: float | None) -> bool:
    ...
    threshold = float(
        per_play_threshold if per_play_threshold not in (None, "") else config.get("trailing", "activation_threshold_pct", default=0) or 0
    )
    if threshold <= 0:
        return True
    trade_type = _get_trade_type(play)
    ...
    stock_tp_enabled = (play.get("take_profit") or {}).get("stock_price_pct") not in (None, 0) or (play.get("take_profit") or {}).get(
        "stock_price"
    ) not in (None, 0)
...
def _append_history(section: dict[str, Any], kind: str, old_level: float | None, new_level: float, reason: str = "recalc") -> None:
...
def _init_on_activation(play: dict[str, Any], current_stock_price: float | None, current_premium: float | None) -> None:
    tp = play.get("take_profit") or {}
...
def update_trailing_levels(play: dict[str, Any], current_stock_price: float | None, current_premium: float | None) -> bool:
    """
...
def get_trailing_tp_levels(play: dict[str, Any]) -> dict[str, float | None]:
