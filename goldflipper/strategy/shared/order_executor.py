...
def get_order_side_for_entry(action: Optional[OrderAction] = None) -> OrderSide:
    """
...
def get_order_side_for_exit(action: Optional[OrderAction] = None) -> OrderSide:
    """
...
class OrderExecutor:
    ...
    def get_option_data(self, option_contract_symbol: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def create_entry_order(
        ...
        if is_limit_order:
            limit_price, price_source = determine_limit_price(
                order_type, option_data, 
                option_data.get('last', 0.0),
                'entry'
            )
            ...
            self.logger.info(
                f"Creating {action_name} limit order: {contract_symbol} "
                f"{qty} @ ${limit_price:.2f} ({price_source})"
            )
            ...
            self.logger.info(
                f"Creating {action_name} market order: {contract_symbol} {qty}"
            )
    ...
    def create_exit_order(
        ...
        if is_limit_order:
            ...
            self.logger.info(
                f"Creating {action_name} exit limit order: {contract_symbol} "
                f"{qty} @ ${limit_price:.2f} ({price_source})"
            )
...
_default_executor: Optional[OrderExecutor] = None
