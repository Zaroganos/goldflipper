...
class StrategyOrchestrator:
    ...
    def __init__(
        ...
        brokerage_client: Optional[Any] = None
    ):
        """
    ...
    def _load_settings(self) -> None:
        ...
        self._execution_mode = (
            ExecutionMode.PARALLEL if mode_str == 'parallel' 
        ...
        for strategy_class in available:
            try:
                ...
                strategy = strategy_class(
                    ...
                    brokerage_client=self._client
                )
                
                if strategy.is_enabled():
                    self.strategies.append(strategy)
                    self.logger.info(
                        f"Loaded strategy: {strategy.get_name()} "
                ...
                self.logger.error(
                    f"Failed to initialize strategy {strategy_class.__name__}: {e}"
                )
    ...
    def run_cycle(self) -> bool:
        ...
        if self.state not in (OrchestratorState.INITIALIZED, OrchestratorState.RUNNING):
            self.logger.warning(
                f"Cannot run cycle: orchestrator state is {self.state.value}"
            )
            return False
        ...
        try:
            ...
            for strategy in self.strategies:
                try:
                    ...
                    self.logger.error(
                        f"Error in {strategy.get_name()}.on_cycle_start(): {e}"
                    )
                    cycle_errors.append({
                        'strategy': strategy.get_name(),
                        'phase': 'on_cycle_start',
                        'error': str(e)
                    })
            ...
            for strategy in self.strategies:
                try:
                    ...
                    self.logger.error(
                        f"Error in {strategy.get_name()}.on_cycle_end(): {e}"
                    )
                    cycle_errors.append({
                        'strategy': strategy.get_name(),
                        'phase': 'on_cycle_end',
            ...
            if success:
                ...
                self.logger.warning(
                    f"Orchestrator cycle {self._cycle_count} completed "
                    f"with {len(cycle_errors)} error(s)"
                )
    ...
    def _run_parallel(self) -> List[Dict[str, Any]]:
        ...
        with ThreadPoolExecutor(max_workers=self._max_workers) as executor:
            ...
            future_to_strategy = {
                executor.submit(self._execute_strategy, strategy): strategy
                for strategy in self.strategies
            }
            ...
            for future in as_completed(future_to_strategy):
                ...
                try:
                    ...
                    self.logger.error(
                        f"Error in parallel execution of {strategy.get_name()}: {e}"
                    )
                    errors.append({
                        'strategy': strategy.get_name(),
                        'phase': 'execute_parallel',
                        'error': str(e)
                    })
    ...
    def _execute_strategy(self, strategy: BaseStrategy) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"[{name}] Found {len(new_plays)} new plays, "
                f"{len(open_plays)} open plays"
            )
            ...
            if new_plays:
                ...
                self.logger.info(
                    f"[{name}] {len(plays_to_open)} plays met entry conditions"
                )
                ...
                for play in plays_to_open:
                    ...
                    try:
                        ...
                        if hasattr(strategy, 'open_position'):
                            success = strategy.open_position(play, play_file)
                            if success:
            ...
            if open_plays:
                ...
                self.logger.info(
                    f"[{name}] {len(plays_to_close)} plays met exit conditions"
                )
    ...
    def run_continuous(
        ...
        market_hours_check: Optional[Callable[[], bool]] = None,
            ...
            'strategies': [
                {
                    ...
                    'config_section': s.get_config_section()
                }
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"mode={self._execution_mode.value}>"
        )
